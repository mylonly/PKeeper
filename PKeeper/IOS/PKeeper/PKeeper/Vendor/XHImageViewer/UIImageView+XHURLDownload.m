...
//    }
    ...
    if (image.size.width  > SCREENWIDTH*SCREENSCALE || image.size.height > SCREENHEIGHT*SCREENSCALE)
    {
        if (image.size.width > image.size.height)
        {
            scaleImage = [image aspectScaleToMaxSize:SCREENWIDTH*SCREENSCALE];
        }
        else
        {
            scaleImage = [image aspectScaleToMaxSize:SCREENHEIGHT*SCREENSCALE];
        }
    }
    NSData* scaleData = UIImageJPEGRepresentation(scaleImage,1.0f);
    if (scaleData)
    {
        [self cachingImageData:scaleData url:url];
    }
    if([url isEqual:self.url]) {
        if(error) {
            self.loadingState = UIImageViewURLDownloadStateFailed;
        } else {
            [self performSelectorOnMainThread:@selector(setImage:) withObject:scaleImage waitUntilDone:NO];
